...
import { Dumbbell, MapPin, ShirtIcon, Waves, Wifi } from "lucide-react";
...
const HotelCard = ({ hotel }: { hotel: HotelWithRooms }) => {
  ...
  return (
    <div
      onClick={() => !isMyHotels && router.push(`/hotel-details/${hotel._id}`)}
      className={cn(
      ...
      <div className="flex gap-2 bg-background/50 border border-primary/10 rounded-lg">
        ...
        <div className="flex-1 flex flex-col justify-between h-[210px] gap-1 p-1 py-2 text-sm">
          <div>
            ...
            <div className="text-primary/90">
            <AmenityItem>
              <MapPin className="w-4 h-4 size-4"/>
              {country?.name} {hotel.state} {hotel.city}
            </AmenityItem>
            {hotel.swimmingPool && <AmenityItem>
              <Waves className="w-4 h-4 size-4"/>
              Pool
            </AmenityItem>}
            {hotel.gym && <AmenityItem>
              <Dumbbell className="w-4 h-4 size-4"/>
              Gym
            </AmenityItem>}
            {hotel.laundry && <AmenityItem>
              <ShirtIcon className="w-4 h-4 size-4"/>
              Laundry
            </AmenityItem>}
            {hotel.freeWifi && <AmenityItem>
              <Wifi className="w-4 h-4 size-4"/>
              Free Wifi
            </AmenityItem>}
          </div>
          <div className="flex items-center justify-between">
            ...
            {isMyHotels && <Button onClick={()=> router.push(`/hotel${hotel._id}`)} variant="outline"> Edit </Button>}
          </div>
          </div>
        </div>
      </div>
